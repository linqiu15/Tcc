...
function pwa_tensor(p, q, usq; cl=:ss)::ComplexF64
    ξ =  (p^2 + q^2 + usq) / (2 * p * q)
    tmp = zero(ComplexF64)
    if cl == :ss
        if iszero(p) || iszero(q)
            ...
            tmp = -1 / (12 * p * q) * (-4 * p * q + (p^2 + q^2 - 2 * p * q * ξ) * (log(ξ - 1) - log(ξ + 1)))
        end
    elseif cl == :sd
        if iszero(p) || iszero(q)
            tmp = -√2 / 3 * p^2 / (p^2 + q^2 + usq)
        else
            tmp = √2 / (12 * p * q) * (q * (-4 * p + 3 * q * ξ) + (p^2 - 2 * p * q * ξ + 1 / 2 * q^2 * (-1 + 3 * ξ^2)) * (log(ξ - 1) - log(ξ + 1)))
        end
    elseif cl == :ds
        if iszero(p) || iszero(q)
            tmp = -√2 / 3 * q^2 / (p^2 + q^2 + usq)
        else
            tmp = √2 / (12 * p * q) * (p * (-4 * q + 3 * p * ξ) + (q^2 - 2 * p * q * ξ + 1 / 2 * p^2 * (-1 + 3 * ξ^2)) * (log(ξ - 1) - log(ξ + 1)))
        end
    else
        if iszero(p) || iszero(q)
            tmp = 0
        else
#             ξ = convert(Complex{BigFloat}, (p^2 + q^2 + usq) / (2 * p * q))  #in fact, this evaluates the precise work but runs  slowly but does not affect the final result
            tmp = -1 / (24 * p * q) * (2 * (6 * p^2 * ξ + 6 * q^2 * ξ - p * q * (2 + 9 * ξ^2)) + (2 * q^2 * (-1 + 3 * ξ^2) + p^2 * (-2 + 6 * ξ^2) + p * q * (ξ - 9 * ξ^3)) * (log(ξ - 1) - log(ξ + 1)))
        end
    end
    return tmp
end

function pwa_central(p, q, usq; cl=:ss)::ComplexF64
    ξ =  (p^2 + q^2 + usq) / (2 * p * q)
    tmp = zero(ComplexF64)
    if cl == :ss
        if iszero(p) || iszero(q)
...
#             ξ = convert(Complex{BigFloat}, (p^2 + q^2 + usq) / (2 * p * q))
            tmp = -1 / (8 * p * q) * (6 * ξ + (-1 + 3 * ξ^2) * (log(ξ - 1) - log(ξ + 1)))
        end
    end
    return tmp
end

function pwa_square(p, q, usq; cl=:ss)::ComplexF64
    ξ =  (p^2 + q^2 + usq) / (2 * p * q)
    tmp = zero(ComplexF64)
    if cl == :ss
        if iszero(p) || iszero(q)
            ...
            tmp = -1 / (4 * p * q) * (-4 * p * q + (p^2 + q^2 - 2 * p * q * ξ) * (log(ξ - 1) - log(ξ + 1)))
        end
    elseif cl == :dd
        if iszero(p) || iszero(q)
            tmp = 0
...
#             ξ = convert(Complex{BigFloat}, (p^2 + q^2 + usq) / (2 * p * q))
            tmp = -1 / (8 * p * q) * (6 * ξ * (p^2 + q^2 - 2 * p * q * ξ) + (p^2 + q^2 - 2 * p * q * ξ) * (-1 + 3 * ξ^2) * (log(ξ - 1) - log(ξ + 1)))
        end
    end
    return tmp
end

function pwa_contact(p, q, usq; cl=:ss)::ComplexF64
    return (cl == :ss) ? 1.0 : 0.0
end


# deformed path integral for pion
