# function pwa_tensor_integral(func;a=1.0)
#     quadgk(x->((-a*im)*func(-1-a*x*im)+2*func(-1-a*im+2*x)+a*im*func(1-a*im+a*x*im)),0,1)[1]
# end
...
#         else
#             tmp =1/6*pwa_tensor_integral(z->(p^2+q^2-2*p*q*z)/(p^2+q^2-2*p*q*z+usq);a=a)
#         end
#     elseif cl == :sd
#         if iszero(p) || iszero(q)
#             tmp = -√2 / 3 * p^2 / (p^2 + q^2 + usq)
#         else
#             tmp=-√2/6*pwa_tensor_integral(z->(q^2/2*(3*z^2-1)+p^2-2*p*q*z)/(p^2+q^2-2*p*q*z+usq);a=a)
#         end
#     elseif cl == :ds
#         if iszero(p) || iszero(q)
#             tmp = -√2 / 3 * q^2 / (p^2 + q^2 + usq)
#         else
#             tmp=-√2/6*pwa_tensor_integral(z->(p^2/2*(3*z^2-1)+q^2-2*p*q*z)/(p^2+q^2-2*p*q*z+usq);a=a)
#         end
#     else
#         if iszero(p) || iszero(q)
#             tmp = 0
#         else
#             tmp=1/6*pwa_tensor_integral(z->(2*(p^2+q^2)*(3*z^2-1)-p*q*z*(9*z^2-1))/(p^2+q^2-2*p*q*z+usq);a=a)
#         end
#     end
#     return tmp
# end
